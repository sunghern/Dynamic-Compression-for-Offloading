...
class ChannelPruning(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.rate <= 0.8:
            ...
            slicing_rate = int(self.out_channels * 0.8)
        ...
        slicing_idx = torch.tensor(slicing_idx).to('cuda:0')
        ...
        if self.rate <= 0.8:
            ...
            #print("nonzero rate :", float(nonzero/nonzero_tot*100))
            #print("value after slicing :", t[1])
            ...
            #self.count_channel += np.where(t.cpu().detach().numpy() != 1.0, 0.0).sum(axix=0)
